...
class Encoder_with_transformer(nn.Module):
    ...
    def __init__(self, encoder_name):
        ...
        self.layer_1 = copy.deepcopy(self.encoder.encoder.layer[0])
        self.pooler = copy.deepcopy(self.encoder.pooler)

        self.config = self.encoder.config

    def forward(self, input_ids, attention_mask, token_type_ids, return_dict=False):
        _, max_seq_len = input_ids.shape
        output = self.encoder(input_ids=input_ids.view(-1, max_seq_len),
                              attention_mask=attention_mask.view(
                                  -1, max_seq_len),
                              token_type_ids=token_type_ids.view(
                                  -1, max_seq_len),
                              return_dict=False)[0]
        output_1 = self.layer_1(output)[0]
        output_2 = self.pooler(output_1)
...
class GuidedMoEBasic(nn.Module):
    def __init__(self, dropout=0.5, n_speaker=2, n_emotion=7, n_cause=2, n_expert=2, guiding_lambda=0, **kwargs):
        ...
        self.emotion_linear = nn.Linear(
            self.bert.config.hidden_size, n_emotion)
        self.n_expert = n_expert
        self.guiding_lambda = guiding_lambda
        self.gating_network = nn.Linear(
            2 * (self.bert.config.hidden_size + n_emotion + 1), n_expert)
    ...
    def binary_cause_classification_task(self, emotion_prediction, input_ids, attention_mask, token_type_ids, speaker_ids):
        ...
        for _ in range(self.n_expert):
            ...
            expert_pred *= gating_prob.view(-1,
                                            self.n_expert)[:, _].unsqueeze(-1)
    ...
    def get_pair_embedding(self, emotion_prediction, input_ids, attention_mask, token_type_ids, speaker_ids):
        ...
        _, pooled_output = self.bert(input_ids=input_ids.view(-1, max_seq_len), attention_mask=attention_mask.view(-1,
...
class PRG_MoE(GuidedMoEBasic):
    def __init__(self, dropout=0.5, n_speaker=2, n_emotion=7, n_cause=2, n_expert=4, guiding_lambda=0, **kwargs):
        super().__init__(dropout=dropout, n_speaker=n_speaker, n_emotion=n_emotion,
                         n_cause=n_cause, n_expert=4, guiding_lambda=guiding_lambda)
...
class PRG_MoE_General(nn.Module):
    def __init__(self, dropout=0.5, n_speaker=2, n_emotion=7, n_cause=2, n_expert=2, guiding_lambda=0, encoder_name='bert-base-cased', **kwargs):
        ...
        # self.encoder = AutoModel.from_pretrained(encoder_name)
        self.encoder = Encoder_with_transformer(encoder_name='bert-base-cased')
        self.emotion_linear = nn.Linear(
            self.encoder.config.hidden_size, n_emotion)
        self.n_expert = n_expert
        self.guiding_lambda = guiding_lambda
        self.gating_network = nn.Linear(
            2 * (self.encoder.config.hidden_size + n_emotion + 1), n_expert)
        self.cause_linear = nn.ModuleList()

        for _ in range(n_expert):
            self.cause_linear.append(nn.Sequential(nn.Linear(2 * (self.encoder.config.hidden_size + n_emotion + 1), 256),
                                                   nn.Linear(256, n_cause)))
