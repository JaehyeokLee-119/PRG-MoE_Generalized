...
class PRG_MoE_General(nn.Module):
    ...
    def get_pair_embedding(self, emotion_prediction, input_ids, attention_mask, token_type_ids, speaker_ids):
        ...
        _, pooled_output = self.encoder(input_ids=input_ids.view(-1, max_seq_len), attention_mask=attention_mask.view(-1, max_seq_len), token_type_ids=token_type_ids.view(-1, max_seq_len), return_dict=False)
        utterance_representation = self.dropout(pooled_output)
