...
class LearningEnv:
    ...
    def set_logger(self, file_name, logger_name):
        ...
        if self.log_directory:
            ...
            file_handler = logging.FileHandler(
                f'{self.log_directory}/{file_name}')
        file_handler.setFormatter(formatter)
        logger.addHandler(file_handler)

    def get_dataloader(self, dataset_file, batch_size, num_worker, shuffle=True, contain_context=False):
        device = "cuda:0"
        data = get_data(dataset_file, device,
                        self.max_seq_len, contain_context)
        utterance_input_ids_t, utterance_attention_mask_t, utterance_token_type_ids_t = data[
            0]
        speaker_t, emotion_label_t, pair_cause_label_t, pair_binary_cause_label_t = data[1:]

        dataset_ = TensorDataset(utterance_input_ids_t, utterance_attention_mask_t, utterance_token_type_ids_t,
                                 speaker_t, emotion_label_t, pair_cause_label_t, pair_binary_cause_label_t)
    ...
    def multiprocess_work(self, test, training_iter, batch_size, learning_rate, patience, num_worker, **kwargs):
        ...
        if self.split_directory:
            self.set_logger_environment(
                [f"{self.model_name}-split_average-{self.start_time}.log"], ["split_logger"])
            logger = logging.getLogger("split_logger")

            split_performance = torch.zeros(
                (3, len(os.listdir(self.split_directory)), 5))
            ...
            for split_dir in os.listdir(self.split_directory):
                ...
                base_file_name = os.path.join(
                    self.split_directory, split_dir, f"split_{self.split_num}")
                ...
                if self.single_gpu:
                    self.child_process(0, training_iter, batch_size, learning_rate,
                                       patience, num_worker, stopper, split_performance, test)
                else:
                    torch.multiprocessing.spawn(self.child_process, args=(
                        training_iter, batch_size, learning_rate, patience, num_worker, stopper, split_performance, test), nprocs=len(self.gpus))

            mean_performance = torch.mean(split_performance, dim=1)
            logger.info(
                f"Emotion Classification Test Average Performance | {len(os.listdir(self.split_directory))} trials | {mean_performance[0]}\n")
            logger.info(
                f"Binary Cause Classification Test Average Performance | {len(os.listdir(self.split_directory))} trials | {mean_performance[1]}\n")
            logger.info(
                f"MultiClass Cause Classification Test Average Performance | {len(os.listdir(self.split_directory))} trials | {mean_performance[2]}\n")

        else:
            if self.single_gpu:
                self.child_process(0, training_iter, batch_size, learning_rate,
                                   patience, num_worker, stopper, None, test)
            else:
                torch.multiprocessing.spawn(self.child_process, args=(
                    training_iter, batch_size, learning_rate, patience, num_worker, stopper, None, test), nprocs=len(self.gpus))
    ...
    def child_process(self, allocated_gpu, training_iter, batch_size, learning_rate, patience, num_worker, stopper, split_performance, test=False):
        ...
        if allocated_gpu == 0:
            ...
            if self.n_cause == 2:
                file_name_list = [
                    f'{self.model_name}-binary_cause-{_}-{self.start_time}.log' for _ in logger_name_list]
            else:
                file_name_list = [
                    f'{self.model_name}-multiclass_cause-{_}-{self.start_time}.log' for _ in logger_name_list]
        ...
        if test:
            self.valid(allocated_gpu, batch_size,
                       num_worker, saver=None, option='test')
        else:
            self.train(allocated_gpu, training_iter, batch_size,
                       learning_rate, patience, num_worker)
            self.valid(allocated_gpu, batch_size,
                       num_worker, saver=None, option='test')
    ...
    def train(self, allocated_gpu, training_iter, batch_size, learning_rate, patience, num_worker):
        ...
        def get_pair_pad_idx(utterance_input_ids_batch, window_constraint=3, emotion_pred=None):
            ...
            for batch in check_pad_idx.view(-1, max_doc_len):
                pair_window_idx = torch.zeros(
                    int(max_doc_len * (max_doc_len + 1) / 2))
                for end_t in range(1, len(batch.nonzero()) + 1):
                    if emotion_pred is not None and emotion_pred[end_t - 1] == 6:
                        continue

                    pair_window_idx[max(0, int(
                        end_t * (end_t + 1) / 2) - window_constraint):int(end_t * (end_t + 1) / 2)] = 1
        ...
        optimizer = optim.Adam(
            self.distributed_model.parameters(), lr=learning_rate)
        ...
        if not os.path.exists("model/"):
            os.makedirs("model/")
        saver = ModelSaver(
            path=f"model/{self.model_name}-{model_name_suffix}-{self.data_label}-{self.start_time}.pt", single_gpu=self.single_gpu)
        ...
        for i in range(training_iter):
            ...
            loss_avg, count = 0, 0
            emo_pred_y_list, emo_true_y_list, cau_pred_y_list_all, cau_true_y_list_all, cau_pred_y_list, cau_true_y_list = [
            ...
            for utterance_input_ids_batch, utterance_attention_mask_batch, utterance_token_type_ids_batch, speaker_batch, emotion_label_batch, pair_cause_label_batch, pair_binary_cause_label_batch in tqdm(train_dataloader, desc=f"Train | Epoch {i+1}"):
                ...
                emotion_prediction = emotion_prediction[(
                    check_pad_idx != False).nonzero(as_tuple=True)]
                binary_cause_prediction_window = binary_cause_prediction.view(
                    batch_size, -1, self.n_cause)[(check_pair_window_idx != False).nonzero(as_tuple=True)]
                binary_cause_prediction_all = binary_cause_prediction.view(
                    batch_size, -1, self.n_cause)[(check_pair_pad_idx != False).nonzero(as_tuple=True)]

                emotion_label_batch = emotion_label_batch.view(
                    -1)[(check_pad_idx != False).nonzero(as_tuple=True)]

                if self.n_cause == 2:
                    pair_binary_cause_label_batch_window = pair_binary_cause_label_batch[(
                        check_pair_window_idx != False).nonzero(as_tuple=True)]
                    pair_binary_cause_label_batch_all = pair_binary_cause_label_batch[(
                        check_pair_pad_idx != False).nonzero(as_tuple=True)]
                else:
                    pair_cause_label_batch = torch.argmax(
                        pair_cause_label_batch.view(-1, self.n_cause), dim=1).view(batch_size, -1)

                    pair_binary_cause_label_batch_window = pair_cause_label_batch[(
                        check_pair_window_idx != False).nonzero(as_tuple=True)]
                    pair_binary_cause_label_batch_all = pair_cause_label_batch[(
                        check_pair_pad_idx != False).nonzero(as_tuple=True)]
                ...
                cau_pred_y_list_all.append(binary_cause_prediction_all), cau_true_y_list_all.append(
                    pair_binary_cause_label_batch_all)
                cau_pred_y_list.append(binary_cause_prediction_window), cau_true_y_list.append(
                    pair_binary_cause_label_batch_window)
                emo_pred_y_list.append(emotion_prediction), emo_true_y_list.append(
                    emotion_label_batch)
    ...
    def valid(self, allocated_gpu, batch_size, num_worker, saver=None, option='valid'):
        ...
        with torch.no_grad():
            valid_dataloader = self.get_dataloader(
                dataset, batch_size, num_worker, shuffle=False, contain_context=self.contain_context)
            ...
            if option == 'test' and allocated_gpu == 0:
                f1_cau = 2 * p_cau * r_cau / \
                    (p_cau + r_cau) if p_cau + r_cau != 0 else 0
    ...
    # conversation: {doc_id:content}, content [[{...}, {...}, ...]]
    def infer(self, conversation):
        with torch.no_grad():
            ...
            emotion_prediction, binary_cause_prediction = self.distributed_model(
                *inputs)
